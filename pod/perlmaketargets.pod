...
environment.  Nothing here has been tested on Windows or VMS.  The tips and
examples presume that you can execute both shell and Perl programs
at the command line, that you have F<make> installed as well as a C-compiler
such as F<gcc>, F<g++> or F<clang>.
...
where F<git> commands do a better job.
...
F<make> targets are specified in a Makefile.  In Perl, like most other
...
directory such as these:
    ...
    ./.config/README        ./generate_uudmap.o
    ./.config/instruct      ./makedepend
    ./Makefile              ./makefile
    ./Policy.sh             ./makefile.old
    ./bitcount.h            ./mg_data.h
    ./cflags                ./myconfig
    ./config.h              ./pod/Makefile
    ./config.sh             ./runtests
...
=head2 F<make test_porting>

F<make test_porting> runs a subset of the tests run via F<make test_harness>:
those test files found in F<t/porting/> in the core distribution.  These tests
enforce standards for coding and metadata, I<e.g.,> correct POD-formatting in
documentation; proper updating of C<$VERSION> in F<*.pm> files, etc.

F<make test_porting> is an important part of our quality assurance and should
be run on every contribution to the core distribution, whether the contributor
is a first-timer or has been submitting patches since 1995.
...
working in a F<git> checkout, you will almost always use F<git clean -dfx>.
