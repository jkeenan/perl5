...
TODO: What does F<make test_prep> do that F<make> does not?

F<make test_prep> creates no new files above and beyond those created by
F<make>. [DUBIOUS! Reinvestigate. make test_prep appears to create 1640 new
files not found in make test_prep_pre.] [It appears that F<make test_prep>,
with one exception, does exactly the same thing as F<make>, albeit in a
slightly different order (which difference needs to be explained, thought not
worried about).]

F<make test_prep> creates a symlink in the F<t/> subdirectory to the newly
generated F<perl> in the top-level directory.  Much of the core distribution
test suite is written on the assumption that the tester is starting from the
F<t/> directory.  Having that symlink enables the tester to issue commands
like this:
