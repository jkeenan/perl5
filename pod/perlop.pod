...
Note that there is an odd corner-case with regard to the underlying reference
type of a blessed object. No unblessed reference is ever C<isa> to any class
name - even its reference type - but a blessed object is.
...
This is done in order to preserve the equivalence to the C<UNIVERSAL::isa>
method, which would also yield true in the same circumstance. Despite this, it
is not true that C<$aobj> would consider methods in the C<ARRAY::> package as
part of its method resolution order.
