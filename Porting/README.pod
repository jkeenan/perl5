...
=head2 F<dist-backcompat-distro-metadata.txt>

Configuration data for F<dist-backcompat.pl>.

=head2 F<dist-backcompat-older-perls.txt>

Configuration data for F<dist-backcompat.pl>.

=head2 F<dist-backcompat.pl>

Program currently under development to test whether changes to modules under
F<dist/> will build and test correctly against older F<perl> releases when
such modules are given a dual-life release to CPAN.
